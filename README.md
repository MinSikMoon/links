...
피앤피시큐어의 DBSAFER(디비세이퍼)는 개인정보보호법을 충족하기 위한 필수 솔루션으로 데이터베이스에 대한 접근통제, 권한통제, 기록보관 및 관리, 암호화 통신, 식별 및 인증에 대한 기능을 제공합니다. 국내 최초로 개발된 Gateway 방식의 DB보안 솔루션으로 Gateway 방식의 장점인 Loss 없는 데이터 로깅과 강력한 접속 통제 기능을 지원하며, 원천적인 정보 유출 방지 및 데이터마스킹 기능을 제공하는 최고의 DB 보안 솔루션입니다.

NODESAFER는 시스템 접근통제 솔루션으로 DB보안제품과 OS보안제품으로 보호하기 힘들었던 시스템에 대한 접근통제 및 감사 기능을 수행합니다. 단독 구성 및 기존의 DB보안 솔루션인 DBSAFER와 연동하여 사용할 수 있는 제품으로 서버상의 필터 형태로 운영되어 서버의 성능저하 없이 처리 되며, 모든 OS보안제품과 충돌이 발생하지 않습니다.
````
#### 디코딩 캐릭터셋들
...
출처: http://visu4l.tistory.com/391 [뭐라 씨부리 쌌노?]
````
#### editplus 에디트플러스 
````
Editplus 가로선택 후 선택영역 채우기http://yoom7ss.egloos.com/5403093

                        Editplus 칸단위 편집 방법
1. 칸단위 영역설정 하기

1-1.Alt+C : 칸단위 영역지정 시작

1-2.커서를 움직여서 역역지정

1-3.Alt+C : 칸단위 영역지정 끝

2. 칸채우기 창 열기

Alt->E->F->F : 칸채우기 창 여는 단축키

(키보드로 하나씩 누른다.

마우스로 메뉴->편집->모양->칸채우기 를 클릭할 수 있지만 이게 더 편하다)

3. 칸단위 편집시작

삽입 또는 바꾸기 등으로 편집하면 된다.

[출처] Editplus 칸단위 편집|작성자 ahn91
````
#### 파일질라 ppk등록
...
### websphere 설명
http://soul0.tistory.com/127?category=543483
